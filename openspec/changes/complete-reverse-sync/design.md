...
target identity를 통과한 operation은 boolean mode가 없는
`xhtml_patcher.apply_validated_patches()`로 적용합니다. validated renderer는
rebased target/anchor가 없거나 source visible text가 base fragment와 다르거나
action이 미등록이면 조용히 skip하지 않고 `PatchApplicationError`로 중단합니다.
offline diagnostic과 raw patch regression fixture는
`legacy_xhtml_patcher.patch_xhtml()`을 사용할 수 있지만 이 API로 push
candidate를 생성할 수 없습니다.
