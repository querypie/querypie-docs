...
#### 2. Download the installation file that matches your PC's operating system.

Download the installation file from Step 1 in the QueryPie Agent Downloads popup window.
Copy the QueryPie URL shown in Step 3.
...
#### 1. Check the connection information for authorized connections in the Database tab within the Agent app.

After login is completed normally, you can check the connection information for authorized connections in the Database tab within the Agent app.
...
Click the `Role` button below the user profile area to choose the desired role and click the `OK` button. When selecting the `Default` role, use the server permissions assigned through Workflow &gt; Server Access Request.
...
#### 2. Connecting to Server through Agent
...
Select the account you want to use, and enter the password if necessary, then click the `OK` button to open the session.
...
##### 3.1. Open a terminal and navigate to the `.ssh` folder.

```
$ cd .ssh
```

##### 3.2. Open the vi editor to create a `config` file in the `.ssh` folder.

```
$ vi config
```

##### 3.3. Enter the following content, then press `Esc` and type `:wq` to save and exit the vi editor.
...
You can also check and modify the location of the `kubeconfig` file and change the file path through the settings menu.
...
4) Copy the command line with `Copy`, declare it in the client, and switch the context to an accessible cluster.
```
