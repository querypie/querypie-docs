...
First, enter `/mcp` to check the status of the currently configured MCP Server.
...
Select 'yes' or 'yes, and don't ask again for querypie -…'.
...
Use the `/mcp auth` command in Gemini CLI to authenticate.
```
 /mcp auth acp
```
