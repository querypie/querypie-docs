...
When operating QueryPie servers on Public Cloud such as AWS, GCP, or Azure, we recommend applying the following configuration.

* Apply High Availability configuration
    * Apply Application Load Balancer and Network Load Balancer for a configuration suitable for high availability and load balancing.
...
* Apply Managed Service MySQL and Redis instead of VMs for QueryPie MySQL and Redis.
    * For AWS, we recommend using Aurora RDS for MySQL, Aurora RDS for MariaDB, and ElastiCache.
* The VM OS supports most Linux distributions. However, you must be able to install and use docker daemon.
