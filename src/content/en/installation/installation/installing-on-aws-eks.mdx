...
Refer to the environment variables guide: [Container Environment Variables](../container-environment-variables)
```ini
...
Register the environment configuration file as a Secret.
```bash
...
Refer to the [Product Versions](../product-versions) document to specify the product version to install.
</Callout>
```yaml
...
Select the Helm version according to the [Product Versions](../product-versions) of QueryPie ACP.
The Chart version in this document is provided as an example.
```bash
helm upgrade --install poc querypie/querypie --version 1.4.4 -n querypie -f values.yaml
```
...
* Proceed with the basic setup procedures in [Installation Guide - setup.v2.sh](installation-guide-setupv2sh). Follow the procedures divided into common settings and product-specific settings.
