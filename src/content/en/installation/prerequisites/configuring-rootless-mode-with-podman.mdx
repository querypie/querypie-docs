...
For reasons why Docker should be used in the Linux distributions below, refer to this document: [Linux Distribution and Docker, Podman Support Status](linux-distribution-and-docker-podman-support-status)
...
For Containers to run automatically when the Linux server restarts, you must register and activate a systemd service that performs the `podman compose up -d` command.
