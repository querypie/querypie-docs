...
3. **Service**: Select the service that will use this Secret Store connection (choose one of Database / Server / Kubernetes). (Kubernetes is supported starting from version 11.5.4.)
    1. Service items cannot be changed after saving.
    2. The available Secret engines by service are as follows.
        1. Database : K/V, Database, Google Cloud
        2. Server : K/V, SSH OTP, SSH CA
        3. Kubernetes : Google Cloud
4. Enter authentication information for integration according to the selected Secret Store type.
    ...
    2. **Auth Method** : Select the authentication method between QueryPie and Vault. It cannot be changed after configuration.
    ...
    3. **Secret Engine** : Select the Secret Engine type of Hashicorp Vault.
        1. Supports Database, K/V, SSH OTP, SSH CA, and Google Cloud.<br/>(Google Cloud secret engine is supported starting from 11.5.6.)
        2. Secret Engine cannot be changed after configuration.
...
5. **Namespace** : Enter the Vault Namespace.
    1. Namespace items cannot be changed after configuration.
...
To actually store authentication information in Secret Store from QueryPie, you must enable Secret Store usage after completing Vault integration.
For detailed information, please refer to the Secret Store configuration documentation.
</Callout>
