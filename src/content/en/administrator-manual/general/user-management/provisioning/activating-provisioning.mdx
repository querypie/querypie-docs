...
2. `Enable` Click the button on the right side of Provisioning to activate SCIM functionality.
3. Store the address value provided as SCIM Endpoint so it can be set as Base URL in the future.
4. `Generate Token` Click the button on the right to issue an Access Token.
...
6. `Confirm` Click the button to close the window.
