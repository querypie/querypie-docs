...
3. Click the `Create SSL` button in the top right.
4. Specify a name in the `Name` field for easy identification.
5. Select `Required` in `SSL Mode`.
6. Open the downloaded bundle certificate (.pem file) in a text editor, copy the entire content, and paste it into the `CA Certificate` field.
...
6. Click the `Save Changes` button in the top right to save the settings.
