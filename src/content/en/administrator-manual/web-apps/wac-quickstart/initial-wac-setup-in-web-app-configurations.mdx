...
4. If there are no separate compatibility information changes received, maintain the existing values for Current Version and Minimum Required Version information.
5. Click the `Save` button to save.
6. You can see that the Extension File link has changed from a link starting with `https://dl.querypie.com/…` to a link starting with the QueryPie Domain you are currently using.
