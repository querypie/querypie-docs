...
4. Wildcards or regular expressions are not supported.
    * `"/*-settings"` (X)
    * `"^/database-settings/policies/data-.*$"` (X)
5. When deny and allow policies overlap for the same resource (and sub-paths), **deny policies take precedence**.
