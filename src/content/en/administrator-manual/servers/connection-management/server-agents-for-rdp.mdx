...
The Host information of Server Agents registered in QueryPie is automatically selected from the IP information that Windows Server has as a communicable Host.
However, if communication from Server Agent to QueryPie is possible but communication from QueryPie to Server Agent is not possible, Server Agent is added and Connection Failed is displayed as Status.
