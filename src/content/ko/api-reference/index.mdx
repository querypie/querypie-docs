...
QueryPie External API 레퍼런스 문서에 오신 것을 환영합니다.
이 문서는 QueryPie Access Control Platform (ACP)과 프로그래밍 방식으로 상호작용할 수 있는 RESTful API에 대한 종합적인 가이드를 제공합니다.

## API 소개

QueryPie External API는 [RESTful API](https://restfulapi.net/) 방식으로 구현된 API입니다.
QueryPie의 주요 기능을 웹 콘솔 외부에서도 사용할 수 있도록 합니다.
현재 **180개 이상의 엔드포인트**를 제공합니다.
QueryPie의 다양한 관리 기능을 프로그래밍 방식으로 제어할 수 있습니다.

QueryPie External API를 사용하면 다음을 수행할 수 있습니다:
...
QueryPie External API를 사용하기 위한 빠른 시작 가이드는 다음과 같습니다:

### 1. API 토큰 획득

모든 API 요청에는 인증이 필요합니다.
QueryPie 관리자로부터 API 토큰을 획득해야 합니다.
API 토큰 생성 및 관리 방법에 대한 자세한 내용은 [API 토큰 문서](/ko/administrator-manual/general/system/api-token)를 참조하세요.
...
API 토큰 생성, 수정, 삭제에 대한 자세한 내용은 [API 토큰 문서](/ko/administrator-manual/general/system/api-token)를 참조하세요.

## 주요 기능 영역

QueryPie External API는 다음과 같은 주요 기능 영역으로 구성되어 있습니다:
...
- **Activity Logs API**: QueryPie 설정 변경 이력 조회
...
- **Security Settings API**: QueryPie 보안 설정 조회 및 구성
...
QueryPie External API는 두 가지 버전을 지원합니다:

### V2 API

- **현재 권장되는 API 버전**입니다.
- QueryPie 버전 9.16.1부터 지원됩니다.
...
- 하위 호환성을 위해 유지됩니다.
- 기존 시스템과의 통합을 위해 계속 지원됩니다.
- 호출 경로: `{querypie url}/api/external/v0.9/`

### 버전 선택 가이드

- 새로운 프로젝트나 통합의 경우 **V2 API** 사용을 권장합니다.
- 기존 시스템과의 호환성이 필요한 경우 V0.9 API를 계속 사용할 수 있습니다.
...
- [External API V2](api-reference/11.4.1/v2) - 현행 API (권장)
- [External API V0.9](api-reference/11.4.1/v0.9) - 레거시 API

각 문서에서는 다음을 확인할 수 있습니다:
...
- [API 토큰 관리](/ko/administrator-manual/general/system/api-token) - API 토큰 생성 및 관리 방법
- [Administrator Manual](/ko/administrator-manual) - QueryPie 관리자 매뉴얼
