...
정책예외(Policy Exception)를 통해 지정하는 상황과 다른 점은 정책 예외는 예외의 기간이 정해져 있지만 Exclusion 대상은 정책과 동일한 수명주기를 갖는다는 것입니다.
