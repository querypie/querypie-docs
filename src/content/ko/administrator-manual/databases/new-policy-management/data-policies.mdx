...
5.  **Sensitive Data Access Monitoring**
    ...
    * 어떤 경로가 민감 정보인지 설정하고, Alert 설정은 General &gt; Company Management &gt; Alert에서 설정합니다.<br/>(11.0.0 기준 신규 정책용 Sensitive Data Access Monitoring Alert 설정 기능은 미구현 상태입니다.)
6.  **DML Query Approval Enforcement**
    * 특정 테이블에 대해 `INSERT`, `UPDATE`, `DELETE` 쿼리를 수행할 경우 Workflow로 승인 절차를 거치도록 강제합니다.
    * 이 정책을 생성하기 전 General &gt; Workflow Management &gt; Approval Rules에서 SQL Request Type의 Approval Rule을 먼저 생성해 두어야 합니다. `INSERT`, `UPDATE`, `DELETE`를 수행하는 것은 수행자에게 해당 privilege가 부여되어 있다는 것을 전제로 합니다. 쿼리를 수행하는 사람이 요청자 본인이 되도록 하려면 아래 그림과 같이 Execution 항목의 Assignee for Execution에 `Allow Assignee selection (All Users)`가 선택되어 있어야 하며, 이 경우 `INSERT`, `UPDATE`, `DELETE` 권한이 있지만 관리자가 아닌 사람도 승인을 거쳐 쿼리를 수행할 수 있습니다.<br/>
      <figure data-layout="center" data-align="center">
      <img src="/administrator-manual/databases/new-policy-management/data-policies/image-20250629-153240.png" alt="General &gt; Workflow Management &gt; Approval Rules에 SQL Request 형식의 Approval rule을 생성" width="419" />
      <figcaption>
    ...
    *  Approval Rule이 생성되어 있으면 아래 그림과 같이 정책과 연계할 Approval Rule을 지정할 수 있습니다.<br/>
      <figure data-layout="center" data-align="center">
      <img src="/administrator-manual/databases/new-policy-management/data-policies/image-20250629-154218.png" alt="Databases&gt;Policy Management&gt;Data Policies에서 SQL Request 형식의 미리 생성한 Approval Rule을 지정" width="494" />
      <figcaption>
      Databases&gt;Policy Management&gt;Data Policies에서 SQL Request 형식의 미리 생성한 Approval Rule을 지정
      </figcaption>
      </figure>
    * DML Query Approval Enforcement는 태그로 대상을 지정할 수 없고 specific data path로 특정 경로를 지정해야 합니다.
...
3.  **Attribute of Users**  : 특정 속성이 있는 사용자에게 동적으로 정책을 적용하려는 경우 사용합니다. <Badge color="grey">11.1.0</Badge>
...
<Callout type="info">
11.3.0에서 모든 정책 유형에 대해 specific data path 선택 시 정규식을 사용할 수 있도록 변경되었습니다.
또한, 대상인 테이블 또는 컬럼을 여러 개 다중으로 선택해서 등록할 수 있도록 변경되었습니다.
</Callout>
...
2.  **Specific data path** : 특정 데이터 경로를 직접 지정하는 옵션입니다. <Badge color="grey">11.0.0</Badge>
    * 이 옵션을 선택한 뒤 Add target 버튼을 눌러 출력되는 팝업 대화창에서 Database, Schema, Table, Column을 지정할 수 있고 정규표현식을 사용할 수도 있습니다.  <br/>  <br/>
      ...
      <figure data-layout="center" data-align="center">
      <img src="/administrator-manual/databases/new-policy-management/data-policies/image-20250629-162448.png" alt="정규식을 사용하여 대상 지정" width="498" />
      <figcaption>
      정규식을 사용하여 대상 지정
      </figcaption>
      </figure>
...
모든 설정이 완료되면 화면 하단의  **Save**  버튼을 클릭하여 정책을 저장합니다.
저장을 취소하려면  **Cancel**  버튼을 클릭합니다.
