...
<img src="/administrator-manual/databases/connection-management/cloud-providers/synchronizing-db-resources-from-google-cloud/image-20241217-123654.png" alt="Administrator &gt; Databases &gt; Connection Management &gt; Cloud Providers" width="760" />
<figcaption>
...
6. 리소스를 동기화하기 위해 필요한 **Credential** 정보를 입력합니다.
    * `Synchronize` 버튼 클릭 시 GCP의 Service Account를 입력하는 수동 동기화 방식을 기본 제공합니다. <br/>QueryPie 10.2.2부터 "Save Credential for Synchronization" 옵션을 제공하여 Credential type으로 `Service Account`를 사용할 때도 스케줄을 통한 동기화가 가능하도록 개선되었습니다.
    * 11.5.6 부터 Hashcorp Vault 의 Google Cloud secret engine을 지원하여 정적 Credential 저장 없이 동기화를 수행할 수 있습니다. Credential type 항목에서 `Vault(GCP)` 를 지정할 수 있습니다.
        * 이 기능을 사용하기 위해서는 사전에  General &gt; System &gt; Integration &gt;  HashiCorp Vault 에 “Vault Secret Store” 설정이 필요하고 (`Roleset Path` 지정을 위해) Vault에 `Role set` 설정 을 해야합니다. <br/>&lt; 참고 &gt; [HashiCorp Vault 연동 정보 입력하기 | HashiCorp-Vault-연동-정보-입력하기](../../../general/system/integrations/integrating-with-secret-store#hashicorp-vault-%ec%97%b0%eb%8f%99-%ec%a0%95%eb%b3%b4-%ec%9e%85%eb%a0%a5%ed%95%98%ea%b8%b0) <br/>&lt; 참고 &gt; [Role set : Hashicorp Vault Google Cloud secret engine](https://developer.hashicorp.com/vault/docs/secrets/gcp#rolesets)
        * Roleset Path는 Vault 설정에 따라 access token 기반 경로와 service account key 기반 경로를 사용할 수 있습니다.
...
<figure data-layout="center" data-align="center">
<img src="/administrator-manual/databases/connection-management/cloud-providers/synchronizing-db-resources-from-google-cloud/image-20241220-094727.png" alt="Save Credential for Synchronization" width="578" />
<figcaption>
...
<figure data-layout="center" data-align="center">
<img src="/administrator-manual/databases/connection-management/cloud-providers/synchronizing-db-resources-from-google-cloud/image-20241220-095231.png" alt="Administrator &gt; Databases &gt; Connection Management &gt; Cloud Providers &gt; Details" width="760" />
<figcaption>
...
3. 우측 상단의 `Synchronize` 버튼을 클릭하면 GCP에서 리소스를 동기화할 수 있습니다.
4. 표시되는 Synchronization Log에서 동기화 진행 상황을 확인할 수 있고, Administrator &gt; General &gt; Systems &gt; Jobs 메뉴에서도 동기화 히스토리를 확인할 수 있습니다.
5. Cloud Provider를 한 번 등록하고 나면 Provider 기본 정보 및 인증 정보는 변경할 수 없습니다.
    1. Name: 변경 가능
    ...
    7. Auto Configuration Upon Initial Synchronization : 변경 불가

<Callout type="important">
"Save Credential for Synchronization" 옵션이 활성화되어 있지 않은 상태로 저장된 동기화 설정은 상세페이지에서 체크박스를 체크하는 것으로 옵션을 활성화할 수 있습니다.
...
2. 표시되는 Service Account JSON 항목에 JSON 키 값을 입력 후 `Ok` 버튼을 클릭합니다.
    * Service Account는 Google Cloud 콘솔 &gt; IAM 및 관리자 &gt; 서비스 계정 메뉴에서 생성 후 JSON 키를 발급하여 사용할 수 있습니다.
3. 해당 JSON 키로 동기화된 리소스는 DB Connections 메뉴에서 확인할 수 있습니다.
