...
QueryPie 에서는 데이터베이스 등록 및 관리를 위한 Google Cloud(GCP) 연동을 지원합니다.
...
6. 리소스를 동기화하기 위해 필요한  **Credential**  정보를 입력합니다.
    * `Synchronize` 버튼 클릭 시 GCP의 Service Account를 입력하는 수동 동기화 방식을 기본 제공합니다. <br/>QueryPie 10.2.2 부터 “Save Credential for Synchronization” 옵션을 제공하여 Credential type로 Service Account를 사용 할 때도 스케줄을 통한 동기화가 가능하도록 개선되었습니다.
...
5. Cloud Provider 를 한번 등록하고 나면 Provider 기본 정보 및 인증 정보는 변경할 수 없습니다.
    1. Name: 변경 가능
...
2. 표시되는 Service Account JSON 항목에 JSON 키 값을 입력 후 `Ok` 버튼을 클릭합니다.
    * Service Account 는 Google Cloud 콘솔 &gt; IAM 및 관리자 &gt; 서비스 계정 메뉴에서 생성 후 JSON 키를 발급하여 사용할 수 있습니다. 
3. 해당 JSON 키로 동기화된 리소스는 DB Connections 메뉴에서 확인할 수 있습니다.

<Callout type="info">
**GCP 에서 Service Account 를 생성할 때 다음의 권한이 필요합니다.**
