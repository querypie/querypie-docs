...
<Callout type="info">
서버 그룹은 Tag 기반으로 서버를 관리할 수 있습니다.
특정 Tag를 서버그룹의 Server Tags에 지정하여, 해당 Tag가 있는 모든 서버가 서버그룹에 포함되는 형태의 유동적인 서버 관리가 가능합니다.
</Callout>


### 서버 그룹 생성/수정하기

아래 정보를 입력하여 서버 그룹을 생성할 수 있습니다.
생성 후 일부 항목은 수정이 가능합니다.
...
* Servers에서는 해당 서버 그룹에 속할 서버를 확인하거나 서버를 서버그룹에 수동 추가할 수 있습니다. 
...
<Callout type="info">
11.3.0에서 Server group에 추가할 서버 목록 팝업에서 태그를 사용하여 대상을 필터링할 수 있게 되었습니다.
<figure data-layout="center" data-align="center">
<img src="/administrator-manual/servers/connection-management/server-groups/managing-servers-as-groups/image-20251010-084037.png" alt="Tag로 Server Group에 추가하려는 대상을 필터링 한 예시" width="674" />
<figcaption>
Tag로 Server Group에 추가하려는 대상을 필터링 한 예시
</figcaption>
</figure>
</Callout>

* Information의 Server Tags에서 수동 추가된 서버와 관련된 태그를 삭제하더라도, 수동으로 추가된 서버는 서버그룹에서 제외되지 않습니다. Servers 테이블에서 체크박스 선택 후 나오는 `Delete` 버튼을 통한 수동 삭제만 가능합니다.
...
1. 해당 커넥션에 접속 시 필요한 Account를 등록하고 관리합니다. Account를 등록하는 방법은 두 가지가 있습니다. 
    ...
    2. 또는 `Add Account` 기능을 이용하여 수동으로 Account 를 추가할 수 있습니다. 
        1. `Add Account` 버튼 클릭 시, 계정 설정을 위한 모달창이 표기됩니다. 선택한 Category마다 입력해야 하는 항목이 다릅니다.  <br/> 
          <figure data-layout="center" data-align="center">
...
4. Category별 설정 항목
    1.  **QueryPie 내부 인증** 
        ...
        3. `QueryPie - SSH Key` : QueryPie에 등록된 SSH Key를 사용하여 자동 로그인합니다.
            ...
            * Admin &gt; Servers &gt; Configurations &gt; Using One Time Accounts 활성화 시, Set As Provisioning Account 체크박스 체크 가능
        4. `QueryPie - Custom Account` : 서버 접속 시 사전에 등록된 계정 대신, 사용자가 계정 정보를 직접 입력하여 접속할 수 있도록 허용합니다. 사용자는 접속 시점에 노출되는 입력 필드에 계정과 패스워드를 수동으로 입력해야 합니다.
            * 입력이 필요한 항목 : Alias
            * Seamless SSH 방식에서는 Custom Account 접속을 지원하지 않습니다.
            * Server Agent가 설치된 Windows Server에서는 Custom Account 접속을 지원하지 않습니다.
            * Server Agent가 없는 Windows Server에서는 Custom Account로만 접속을 할 수 있습니다.
    2.  **Provisioning 및 One Time Account** 
        1. Admin &gt; Servers &gt; Configures에서 Password Provisioning과 Using One Time Accounts가 활성화되어야 카테고리 항목이 보입니다. 
    ...
    3.  **Secret Store 연동 (Hashicorp Vault)** 
        1. Admin &gt; General &gt; Security &gt; Secret Stores가 활성화되어야 카테고리 항목이 보입니다. 
        ...
        4. `Vault - SSH - OTP` **** : Vault의 SSH OTP 엔진을 사용합니다.
            ...
            * Hashicorp Vault의 SSH OTP 엔진을 사용하여, Passwordless 형태로 서버 계정을 관리할 수 있습니다. 단, 서버에 vault의 server agent인 vault-ssh-helper를 설치해야 합니다. 자세한 가이드는 [Hashicorp Vault 공식 가이드](https://developer.hashicorp.com/vault/tutorials/secrets-management/ssh-otp)를 참조해 주세요.
        5. `Vault - SSH - CA` **** : Vault의 SSH CA 엔진을 사용합니다.
            * 입력 및 선택이 필요한 항목: Account, Secret Store, Vault Role Path
                * Secret Store : 엔진타입이 CA 타입인 항목만 필터되어 표기됩니다. 
            * Hashicorp Vault의 SSH CA 엔진(Signed SSH certificates)을 사용하여, Passwordless 형태로 서버 계정을 관리할 수 있습니다. 단, vault ca 엔진에서 발급한 Public Key를 접속할 서버의 TrustedUserCAKeys에 추가해야 합니다. 자세한 가이드는 [Hashicorp Vault 공식 가이드](https://developer.hashicorp.com/vault/docs/secrets/ssh/signed-ssh-certificates)를 참조해 주세요.
...
<Callout type="info">
**참고: QueryPie - Custom Account**
* Seamless SSH 접속 방식에서는 이 기능을 지원하지 않으며, 접속 시도 시 "Custom account not supported on seamless SSH" 오류 메시지가 표시됩니다.
* Admin &gt; Audit &gt; Server Access History에는 접속에 사용된 Alias가 기록됩니다. SSH 접속의 경우 사용자가 입력한 계정명도 함께 기록되지만, Server Agent가 없는 Windows RDP 접속 시에는 Alias만 기록됩니다.
</Callout>

<Callout type="info">
Provisioning Account는 실제 서버에 등록된 계정이어야 하며 아래와 같은 설정이 필요합니다.
* 계정 생성/삭제 권한이 필수입니다.
...
*  **TTL / Max TTL**  설정 시, 60 이상으로 설정합니다.
    * QueryPie에서는 인증서의 TTL(Time To Live)이 60초입니다.
*  **Signing Algorithm** 은 rsa-sha2-256으로 설정합니다.
</Callout>
...
<Callout type="info">
Workflow에서 보여지는 Server Group을 제한하기 위해선 Admin &gt; Servers &gt; General &gt; Configurations에서 "Show Server Groups in Workflow if Assigned as Member" 항목을 활성화해야 합니다.
자세한 위치는 [Server Access Request Default Settings | 기본-서버-접근-정책-설정](../../sac-general-configurations#%ea%b8%b0%eb%b3%b8-%ec%84%9c%eb%b2%84-%ec%a0%91%ea%b7%bc-%ec%a0%95%ec%b1%85-%ec%84%a4%ec%a0%95) 가이드를 참고해 주세요.
</Callout>
