...
AWS 내의 리소스를 동기화하여 QueryPie에서 관리하는 서버로 등록하고, 사용자 및 그룹에 동기화 해온 서버에 대한 접근 권한을 부여하고 정책을 설정할 수 있습니다.
...
A. 이미  **Creadential** 이 `Default Credentials`이면서 동일한 `Region`으로 등록된 Cloud Provider가 있는 경우 중복으로 등록이 불가합니다.
이 경우 다른 `Region`을 선택하여 등록하셔야 합니다.
</Callout>
