...
3. **Service**: 이 Secret Store의 연결을 사용할 서비스를 선택(Database / Server / Kubernetes 중 택 1)합니다. (11.5.4 이상 버전부터 Kubernetes 지원이 가능합니다.)
    1. Service 항목은 저장 후 변경이 불가능합니다.
    2. Service 별 선택 가능한 Secret engine은 다음과 같습니다.
...
4. 선택된 Secret Store 타입에 따라 연동을 위한 인증 정보를 입력합니다.
    ...
    2. **Auth Method** : QueryPie 와 Vault 의 인증 방식을 선택합니다. 설정 후 변경이 불가능합니다.
    ...
    3. **Secret Engine** : Hashicorp Vault 의 Secret Engine 타입을 선택합니다.
        1. Database, K/V, SSH OTP, SSH CA, Google Cloud 4가지를 지원합니다.<br/>(11.5.6 부터 Google Cloud secret engine을 지원합니다.)
        2. Secret Engine 은 설정 후 변경이 불가능합니다.
        3. 변경하고자 하는 경우 모든 커넥션의 Secret Store 설정을 해제한 뒤 재설정이 필요합니다.
5. **Namespace** : Vault의 Namespace를 입력합니다.
    1. Namespace 항목은 설정 후 변경이 불가능합니다.
6. 필요한 모든 정보를 입력했다면, **Verify integration** 버튼을 클릭합니다.
    1. 모든 정보가 올바르게 입력되었다면 ✔️ Success 메시지가 표시됩니다.
7. `OK` 버튼을 클릭하여 저장합니다.

<Callout type="important">
실제로 QueryPie에서 Secret Store에 인증 정보를 저장하려면, Vault 연동을 마친 후에 Secret Store 사용을 활성화해야 합니다.
자세한 내용은 Secre Store 설정 문서를 참고하세요.
</Callout>
