...
Secret Store를 통해 안전하게 DB 및 서버 인증 정보를 관리할 수 있고, 사용자에게 직접적인 인증 정보 노출 없이 DB 및 서버에 안전하게 접속이 가능합니다.
커넥션 접속 정보 저장 시 인증 정보를 QueryPie 내부에 저장하는 것이 아닌 Secret Store에서 가져와 인증할 수 있도록 설정할 수 있습니다.

실제 [Secret Store의 사용](../../company-management/security) 여부는 Security 메뉴에서 설정할 수 있습니다.
...
1.  **QueryPie 연동할 Vault 서버:** 
    * Vault와 연동하기 위해서는 해당 Vault 서버가 동작하고 있어야 합니다. Vault 서비스가 실행되고, API 요청을 처리할 수 있고, Vault에서 인증 처리를 수행할 수 있어야 합니다.
2.  **Vault에 저장된 리소스의 인증 정보:** 
    * Vault와 연동하기 위해서는 Vault에 접속할 서비스(ex. DB, Server 등)의 인증 정보를 미리 저장해두어야 합니다. 연동하려는 서비스에 대한 접근 권한과 인증 정보(ex. Username, Password, API 키 등)를 Vault에 저장한 상태를 의미합니다.
...
자세한 내용은 Security | Secret Store 설정 문서를 참고하세요.
</Callout>
...
4. 선택된 Secret Store 타입에 따라 연동을 위한 인증 정보를 입력합니다.
    ...
    2.  **Auth Method**  : QueryPie와 Vault의 인증 방식을 선택합니다.
    ...
    3.  **Secret Engine**  : Hashicorp Vault의 Secret Engine 타입을 선택합니다. 
        1. 현재 버전 기준 Database, K/V, SSH OTP, SSH CA 엔진 4가지를 지원합니다.
        2. Secret Engine은 저장 후 변경이 불가능합니다. 
...
<Callout type="info">
**Active Directory 연동 시 사전 준비 사항:**
