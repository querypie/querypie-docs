...
*  **Message Sending Method**  : 메시지 전송 방식
    * API : Slack API를 활용한 메시지 전송
        * API를 선택한 경우, New Request Alert에서 승인자(assignee) 멘션이 가능합니다. (10.2.2 버전 이후)
...
3. 위 화면에서 알림을 보낼 App Name과 Slack Workspace를 선택합니다.
...
1. 생성했던 App을 클릭하여 앱의 Settings 페이지에 진입한 뒤, `App Manifest` 메뉴로 이동합니다.
...
3. 상단의 노란색 알림 바에서 `Reinstall your app`을 클릭하여 앱을 재설치합니다. (필수)
4. `Reinstall to {Workspace_name}` 버튼을 클릭하여 알림을 보낼 Slack 채널을 선택한 뒤 `허용` 버튼을 클릭합니다.
    1. Scope를 변경한 뒤 적용하려면, 이 단계를 다시 수행해야 합니다.
5. Slack 채널로 이동하여 생성한 앱을 추가합니다. (이미 앱을 추가한 상태라면 이 단계는 건너뛸 수 있습니다)
