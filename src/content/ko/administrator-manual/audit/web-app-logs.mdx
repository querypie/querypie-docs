...
QueryPie에서의 모든 웹앱을 감사 로그에 기록합니다.
사용자와 관리자의 로그인/로그아웃, 권한 부여와 회수, 웹애플리케이션에 대한 수행 이력, 리소스 또는 보안 설정 변경 등 QueryPie와 접근제어 대상 리소스에서 발생하는 모든 활동 이력이 로그의 대상입니다.
실시간으로 기록되는 로그에 대해 다양한 조건으로 검색 및 필터링이 가능합니다.

### 연관 감사 로그 종류

웹앱과 공통으로 제공되는 감사 로그에 더하여 웹앱 접근제어 (QueryPie WAC) 라이선스 보유 시, 아래와 같은 감사 로그를 제공합니다.
