...
<tr>
<td>
Security Configuration Status <Badge color="blue">updated-10.2.2</Badge><br/>보안 설정 현황
</td>
<td>
X
</td>
...
11.  **Last Report File**  : 버튼을 클릭하면, 마지막으로 생성된 보고서 파일을 다운로드 받을 수 있습니다.
    ...
    3. 비밀번호 지정 활성화 시, 파일을 다운로드할 때 비밀번호를 설정합니다. 이후 파일의 압축을 해제할 때 설정한 비밀번호를 입력해야 합니다.
...
9.  **Report List 탭**  : 생성된 보고서의 목록을 표시합니다.
    ...
    5. Report File : 파일 다운로드 링크
        ...
        * 비밀번호 지정 활성화 시, 파일을 다운로드할 때 비밀번호를 설정합니다. 이후 파일의 압축을 해제할 때 설정한 비밀번호를 입력해야 합니다.
...
1.  **User/Group**  : 보고서 출력 데이터를 사용자 또는 그룹으로 필터링합니다.
    1. 예: PM팀에 대해 부여된 DB 접근 권한 보고서 출력을 원하는 경우 다음과 같이 선택
        * Type: QueryPie DAC
...
2.  **Resource**  : 보고서 출력 데이터를 리소스에 부여된 태그로 필터링합니다.
    ...
    3. Workflow의 경우 한 번에 여러 개의 리소스에 대해 요청이 가능합니다. 이때 요청 내에 조건을 만족하는 리소스가 한 개 이상 포함되었다면 검색 결과에 포함됩니다.
...
4.  **Time Range**  : DB Access History 또는 Server Access History 선택 시 시간 범위로 필터링합니다.
    ...
    2. 입력된 시간 범위 내의 결과를 제외할지 여부를 선택합니다. 선택 시, 입력된 시간 범위 외의 로그만 출력됩니다.
