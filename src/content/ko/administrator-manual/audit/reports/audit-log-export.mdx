...
3. **Download File Format**: 로그 출력 파일 형식을 지정합니다. (9.17.0 기준, 각 로그마다 다운로드 가능한 형식이 단일로 제한됩니다. 상단의 **'추출 지원 로그 종류'** 을 참고하시기 바랍니다.)
...
7. **Filter Expression** : 필터 표현식을 지정합니다.
    1. 조건 입력 방법 및 예시는 하단의 '**필터 표현식**'을 참고하시기 바랍니다.
...
(1) 필터 표현식을 사용하기 위해 'See Log Template and Description'을 통해 로그별 key와 각각의 type 및 포함하는 value를 참고하시기 바랍니다.<br/>
...
만료된 로그 파일이 필요해진 경우, 로그 추출 태스크를 다시 생성하시기 바랍니다.
