...
2. 다음 설정을 `config` 파일에 추가한 후 저장하고 종료합니다. (저장하려면 `:wq`를 누르세요)
```
Host {{Server Name}}
  Hostname {{Server URL}}
...
3. 서버 간 URL과 포트가 겹치지 않는 경우 아래와 같이 설정할 수도 있습니다.
```
Host *
  ProxyCommand qpctl ssh-proxy %r %h %p develop.dev.querypie.io
```

4. 간소화된 SSH 명령어를 사용하여 서버에 접속합니다.
```
...
2. 다음 줄을 쉘 설정 파일에 추가한 후 저장하고 종료합니다. (저장하려면 `:wq`를 누르세요)
```
alias qshdev='ssh -o ProxyCommand="qpctl ssh-proxy %r %h %p"'
```

3. 변경 사항을 현재 쉘 세션에 적용합니다.
...
2. `~/.ssh/config` 에 설정한 것도 `qpctl` 만 붙여주세요.
```
$ cat ~/.ssh/config
