...
기존의 `ssh` 명령어를 사용하기 위해 SSH config 파일을 설정할 수 있습니다.
...
기존의 `ssh` 명령어를 사용하기 위해 SSH Alias를 설정할 수 있습니다.
...
Seamless SSH는 QueryPie Multi Agent 설치 시 함께 설치되는 다음 명령어를 이용하여 구동됩니다.
```
...
연결 단계에서 QueryPie Proxy 서버 연결에 오류가 발생하면 기존 User Agent는 dialog를 표시하고 있어 터미널 친화적이지 않았습니다.
...
`qpctl` 명령에 사용하는 기본 서버를 지정하는 기능이 추가되었습니다.

* 기본 서버가 지정되면  **qpctl ssh-proxy**  명령에서  **QueryPie Host**를 입력하지 않아도 됩니다.
* 기본 서버가 지정되어 있더라도  **QueryPie Host**를 입력하면 이 값을 사용합니다.

<Callout type="info">
Reset All Settings으로 앱을 초기화하면 기본 호스트 설정도 삭제됩니다.
</Callout>
...
UI에서 현재 선택한 QueryPie Host가 아닙니다.
</Callout>
```
$ qpctl host list
QueryPie Hosts:
...
현재 등록된 QueryPie Hosts 목록을 표시하고 방향키로 선택합니다.
```
$ qpctl host use
...
1. 사용자가 사용하고자 하는 ssh 명령어 앞에 `qpctl`만 붙이세요.
```
$ qpctl ssh ec2-user@core.dev.querypie.io -i ~/.ssh/querypie-dev.pem
```


1. `~/.ssh/config`에 설정한 것도 `qpctl`만 붙이세요.
```
$ cat ~/.ssh/config
...
qpctl이 ProxyCommand를 사용하기 때문에 **사용자 지정 ProxyCommand**는 사용할 수 없습니다.
```
