...
사용자는 권한이 있는 서버의 목록을 서버, 서버 그룹별로 또는 Cloud Provider별로 정렬하여 한눈에 확인할 수 있습니다.
...
2. 서버 목록에서 서버를 선택하면 우측 페이지에서 서버 및 계정 정보를 확인할 수 있습니다.
    ...
    6.  **Password** (또는 SSH Key) : 선택한 계정의 인증 방식이 Password 일 경우 Password를 입력할 수 있는 필드가 표시됩니다. 만약 자동 로그인 설정이 되어 있다면 Password 항목이 표시되지 않으며, 관리자가 설정한 패스워드 또는 SSH Key로 로그인을 하게 됩니다.
