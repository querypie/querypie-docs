...
* Workflow API (기존 Access Approval API와 Approval API를 대체합니다.)
    ...
    * [SQL Request] Reject
    * All Requests
* Approval Rule API V2 (기존 Approval Rule API를 대체합니다.)
    * List
    * Add approval rule
...
* Query Parameter로 사용할 수 있는 `status`가 변경되었습니다. `PARTIALLY_APPROVED`가 `IN_PROGRESS`로 대체되었고, `NONE`이 삭제되었습니다.
