...
* Application_CPU_Count - QueryPie Server Container 가 실행되는 Linux VM 의 vCPU 개수
* Application_Node_Count - QueryPie Server Container 의 개수, 또는 실행되는 Linux VM 의 개수, 또는 Kubernetes Pod 의 개수.
    * 이중화 구성의 경우, 2, 삼중화 구성의 경우, 3 이 됩니다.
* RDS_CPU_Count - 별도로 구성된 QueryPie MySQL 이 실행되는 VM Instance 의 vCPU 개수
...
이 문제 현상은 QueryPie MySQL이 허용하는 연결수보다 QueryPie Server가 요구하는 DB 연결수가 더 큰 경우에 발생합니다.
...
<img src="/installation/container-environment-variables/optimizing-dbmaxconnectionsize/Screenshot-2025-04-24-at-9.38.20-PM.png" alt="[API] Could not open JPA EntityManager for transaction" width="490" />
<figcaption>
...
#### 문제해결방법 1) QueryPie MySQL 이 허용하는 연결수를 늘리기

두 가지 설정값을 500 이상으로 늘리는 설정을 적용할 수 있습니다.
이를 통해 QueryPie Server가 요구하는 DB 연결수보다 QueryPie MySQL이 허용하는 연결수를 더 크게 설정할 수 있습니다.
...
이를 통해 QueryPie MySQL이 허용하는 연결수보다 QueryPie Server가 요구하는 DB 연결수를 더 작게 설정할 수 있습니다.
