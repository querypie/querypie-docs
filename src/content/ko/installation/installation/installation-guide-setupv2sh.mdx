...
1. `docker-compose.yml` , `.env` 등 Compose 를 위한 설정파일을 내려받기
    * 기존의 `compose-env` 라는 환경변수 설정 파일의 이름이 `.env` 로 바뀌었습니다. `compose-env` 는 설정파일 생성을 위한 template 으로 사용됩니다.
2.  `.env` 에 환경변수 값을 설정하기
    * 각 환경변수에 대한 설명은 [컨테이너 환경변수](../container-environment-variables) 문서를 참고해 주세요.
3. docker image 내려받기
...
`setup.v2.sh`가 querypie-app container를 정상적으로 기동하고 나면, 다음과 같은 실행 성공 안내 메시지를 만나게 됩니다.
```
...
`setup.v2.sh`의 실행 마지막에 보이는 `http://172.31.11.201:8000`과 같은 URL에 접속을 시도해 보십시오.
