...
두 설치 프로그램의 작동 방식, 특징과 차이점을 비교합니다.

## 요약

| 구분 | setup.sh | setup.v2.sh |
| --- | --- | --- |
| 대상 | 파트너, 기존 고객 | 신규 고객, Community Edition |
| 자동화 수준 | 낮음 (수동 설정 필요) | 높음 (전체 과정 자동화) |
| Container Registry | Harbor (계정 필요) | Docker Hub (인증 불필요) |
| Rootless Mode | 추가 설정 필요 | 기본 지원 |
| 구버전 지원 | Version 10 이전 지원 | Version 11 이상만 지원 |
...
* <Badge color="yellow">검토 중</Badge> 환경변수 설정파일 `.env` 의 설정값을 이용자 확인, 입력을 거쳐 채웁니다.
...
setup.v2.sh와 관련된 문제 현상을 신고할 때에는, 다음의 두 가지 정보를 포함하여 신고해 주세요.

* setup.v2.sh 실행 시 옵션을 포함한 명령줄, 스크립트 버전을 포함한 콘솔 로그 앞부분 5줄
* 문제 현상이 발생한 마지막 콘솔 로그 5줄 또는 관련 내용

### 스크립트 버전을 포함한 콘솔 로그 예시
