...
A. QueryPieではQueryPie Agent (User Agent / Multi-Agent)を通じてのみWindows Serverに接続できます。
接続方法は以下の通りです。
...
A. ユーザーのPCがmacOSの時、QueryPie User Agentを通じてmacOSに接続できます。
接続方法は以下の通りです。
