...
`setup.v2.sh`は次の主要インストール手順を自動的に実行します。
...
ネットワーク接続構成を参照してFirewall、AWS Security Group設定などを変更する必要があるかもしれません：[システムアーキテクチャとネットワークアクセス制御](../system-architecture-and-network-access-control)
...
インストールが完了した後、運用環境に合わせて初期設定を進める必要があります。
