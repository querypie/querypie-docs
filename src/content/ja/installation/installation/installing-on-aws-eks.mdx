...
querypie:
  ...
  ingress:
    ...
    host: querypie.querypie.io # 実際に設定されたURLを入力
  ...
  resources:
    ...
    limits:
      ...
      memory: 16Gi
  # -- Logrotator設定
  # バージョン11.3.0からログローテーションはメインコンテナに組み込まれています。
  # logrotatorイメージはHarborレジストリでのみ利用可能です。
  # Docker Hub使用時はこれを無効にする必要があります。
  logrotator:
    enabled: false
    # Harbor registry使用時は以下のコメントを解除してenabled: trueに設定してください：
    # image:
...
config:
  ...
  externalURL: "https://querypie.querypie.io" # 実際に設定されたURLを入力
...
* [インストールガイド - setup.v2.sh](installation-guide-setupv2sh)の基本設定手順を進行します。共通設定、製品別設定に分かれた手順を進行します。
