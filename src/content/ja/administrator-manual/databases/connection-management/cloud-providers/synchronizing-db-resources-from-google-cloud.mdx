...
QueryPieではデータベース登録および管理のためのGoogle Cloud（GCP）連動をサポートしています。
GCP内のリソースを同期してQueryPieで管理するデータベースとして登録し、ユーザーおよびグループに同期して持ってきたデータベースに対するアクセス権限を付与し政策を設定できます。
...
3. **Name** 項目に該当プロバイダーを区分できる名前を入力します。
...
6. リソースを同期するために必要な  **Credential** 情報を入力します。
    ...
    * 11.5.6からHashiCorp VaultのGoogle Cloud secret engineをサポートし、静的Credentialを保存せずに同期を実行できます。Credential type項目で`Vault(GCP)`を指定できます。
        * この機能を使用するには、事前にGeneral &gt; System &gt; Integration &gt; HashiCorp Vaultで「Vault Secret Store」設定が必要であり、`Roleset Path`を指定するためにVaultで`Role set`設定を行う必要があります。<br/>&lt; 参考 &gt; [HashiCorp Vault連携情報の入力 | HashiCorp-Vault連携情報の入力](../../../general/system/integrations/integrating-with-secret-store#hashicorp-vault%e9%80%a3%e6%90%ba%e6%83%85%e5%a0%b1%e3%81%ae%e5%85%a5%e5%8a%9b) <br/>&lt; 参考 &gt; [Role set : Hashicorp Vault Google Cloud secret engine](https://developer.hashicorp.com/vault/docs/secrets/gcp#rolesets)
...
3. 右上の`Synchronize`ボタンをクリックするとGCPからリソースを同期できます。
...
5. Cloud Providerを一度登録した後はProvider基本情報および認証情報は変更できません。
    ...
    7. Auto Configuration Upon Initial Synchronization: 変更不可
