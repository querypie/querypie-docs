...
組織で管理するKubernetesクラスターのAPIサーバー呼び出し中、Pod Execなどの接続セッション内行為を記録します。
管理者は映像再生を通じてユーザーが運用コンテナシェルでの実行履歴をモニタリングできます。
...
7. 各行のAction Atのリンクをクリックするとセッションレコーディング内訳の再生が可能です。
8. Action Atのリンクをクリックするとモーダルが実行され、再生のための初期ダウンローディングプロセスを見せるモーダルが現れます。
  <figure data-layout="center" data-align="center">
...
9. 録画されたファイルのサイズが700MBを超過するかどうかによって再生画面を露出するかダウンロードボタンを露出します。
    1.  **700MB未満**
      <figure data-layout="center" data-align="center">
    ...
    2.  **700MB以上**
      ...
      <img src="/administrator-manual/audit/kubernetes-logs/pod-session-recordings/image-20240618-053333.png" alt="image-20240618-053333.png" width="434" />
        ...
        2. `Download`ボタンを押すとダウンロードが進行してローカルにファイルが生成されます。
            1. Administrator &gt; General &gt; Company Management &gt; Security &gt; Others に位置する `Export a file with Encryption`共通オプションが`Required`でアクティブになっている場合、該当ダウンロード時に暗号化モーダルが追加で現れ、該当モーダル以降にダウンロードを実行します。  <br/>
              <figure data-layout="center" data-align="center">
