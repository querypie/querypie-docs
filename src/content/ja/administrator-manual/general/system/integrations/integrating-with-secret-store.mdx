...
3. **Service**: このSecret Storeの接続を使用するサービスを選択（Database / Server / Kubernetesのいずれか）します。（11.5.4以降、Kubernetesをサポートしています。）
    1. Service項目は保存後変更できません。
    2. Service別に選択可能なSecret engineは以下の通りです。
...
4. 選択されたSecret Storeタイプにより連携のための認証情報を入力します。
    ...
    2. **Auth Method** : QueryPieとVaultの認証方式を選択します。設定後は変更できません。
        1. **Token** : Vaultで発行されたTokenを利用した認証方式です。
        2. **AppRole** : Role IDとSecret IDの組み合わせを利用した認証方式です。
            1. **Role ID** : AppRole認証時に必須で使用される固有識別子です。
            2. **Secret ID** : ログイン時に必要な秘密Credentialsです。
    3. **Secret Engine** : Hashicorp VaultのSecret Engineタイプを選択します。
        1. Database、K/V、SSH OTP、SSH CA、Google Cloudをサポートしています。<br/>（11.5.6からGoogle Cloud secret engineをサポートしています。）
        2. Secret Engineは設定後変更できません。
        3. 変更したい場合はすべてのコネクションのSecret Store設定を解除した後再設定が必要です。
5. **Namespace** : VaultのNamespaceを入力します。
    1. Namespace項目は設定後変更できません。
6. 必要なすべての情報を入力したら、**Verify integration**ボタンをクリックします。
    1. すべての情報が正しく入力されていれば、✔️ Successメッセージが表示されます。
7. `OK`ボタンをクリックして保存します。

<Callout type="important">
実際にQueryPieでSecret Storeに認証情報を保存するには、Vault連携を完了した後Secret Store使用を有効化する必要があります。
詳細内容はSecret Store設定ドキュメントを参照してください。
</Callout>
...
1. `Admin > General > System > Integrations > Secret Stores`メニューに進入します。
...
<Callout type="info">
AWS Secrets Managerに関する詳細は[AWS公式ドキュメント](https://docs.aws.amazon.com/secretsmanager/latest/userguide/intro.html)を参照してください。
</Callout>

1. `Admin > General > System > Integrations > Secret Stores`メニューに移動します。
2. AWS Secrets Managerタイルを選択します。
3. `+ Connect`ボタンをクリックします。
4. Secret Store作成のための情報を入力します。
    * **Name** : Secret Storeの名称を入力します。
    * **Region** : AWS Regionを選択します（例：ap-northeast-2）。
    * **Auth Method** : 認証方式を選択します。
        1. **IAM Role** : QueryPie ACP実行環境に構成されたデフォルトのAWS資格証明を使用します。別途の認証情報入力は必要ありません。
        2. **Access Key** : AWS IAM Access Keyで認証します。以下の項目を追加で入力します。
            * **Access Key ID** : AWS IAM Access Key IDを入力します。
            * **Secret Access Key** : AWS IAM Secret Access Keyを入力します。
        3. **Assume Role** : Role ARNを使用してAssume Role方式で認証します。必要に応じてCross-accountアクセスにも使用できます。以下の項目を追加で入力します。
            * **Role ARN** : IAM Role ARNを入力します（例：`arn:aws:iam::123456789012:role/MyRole`）。
            * **External ID** :（任意）Cross-accountアクセスに関連するExternal IDを入力します。
5. `Verify Integration`ボタンをクリックしてAWS接続状態を検証します。
6. `OK`ボタンをクリックして保存します。

<Callout type="important">
AWS Secrets Managerにアクセスするためには、IAMプリンシパル（ユーザーまたはRole）に`secretsmanager:DescribeSecret`、`secretsmanager:GetSecretValue`権限が必要です。
Assume Roleを使用する場合、対象Roleにも該当権限が付与されている必要があります。
AWS IAMに関する詳細は[AWS公式ドキュメント](https://docs.aws.amazon.com/IAM/latest/UserGuide/introduction.html)を参照してください。
</Callout>
...
1. `Admin > General > System > Integrations > Secret Stores`メニューに移動します。
2. AWS Secrets Managerタイルを選択します。
3. 登録されたSecret Storeリストが表示されます。リストのフィールドに関する説明は以下の通りです。
...
1. `Admin > General > System > Integrations > Secret Stores`メニューに移動します。
2. AWS Secrets Managerタイルを選択します。
3. 詳細画面で削除対象をチェックした後`Delete`ボタンをクリックします。
