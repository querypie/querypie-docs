...
<img src="/administrator-manual/general/system/integrations/integrating-with-splunk/screenshot-20240613-174947.png" alt="Splunk連携推奨アーキテクチャ" width="760" />
<figcaption>
Splunk連携推奨アーキテクチャ
</figcaption>
</figure>
...
<figure data-layout="center" data-align="center">
<img src="/administrator-manual/general/system/integrations/integrating-with-splunk/image-20240523-101104.png" alt="（左）TCP / UDP設定画面 （右）HTTP / HTTPS設定画面" width="760" />
<figcaption>
（左）TCP / UDP設定画面 （右）HTTP / HTTPS設定画面
</figcaption>
</figure>

4. Destination情報を生成するために以下の情報を入力します。
...
6. 送信開始をするにはページ左上部にあるトグルボタンを`ON`に切り替えます。
    1. この送信トグルボタンはメンテナンスなど多様な状況で一時的に送信を停止する必要がある場合使用できます。
7. もしもはやSyslog送信が必要ない場合`Delete`ボタンを通じて設定を削除できます。
