"""Byte-equal verification for lossless roundtrip."""
...
def iter_case_dirs(testcases_dir: Path) -> Iterable[Path]:
    for child in sorted(testcases_dir.iterdir()):
        if not child.is_dir():
            continue
        if (child / "expected.mdx").exists() and (child / "page.xhtml").exists():
            yield child
