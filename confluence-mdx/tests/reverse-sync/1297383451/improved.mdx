...
아래의 리눅스 배포본에서 Docker를 사용해야 하는 이유에 대해서는 이 문서를 참조하세요: [리눅스 배포본과 Docker, Podman 지원 현황](linux-distribution-and-docker-podman-support-status)
...
기본 제공되는 `./querypie/<version>/compose.yml` 을 이용해, 하나의 리눅스 서버에서 MySQL 를 실행하지 않고, 별도로 구성된 MySQL을 사용하는 경우, systemd service 작동 범위를 변경하면 됩니다.
