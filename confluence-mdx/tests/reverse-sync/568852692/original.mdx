...
인스턴스 endpoint로 접속하는 것보다 Cluster endpoint 를 사용하는 것을 권고합니다.
...
3. 우측 상단에 있는 `Create SSL` 버튼을 누릅니다.
4. `Name` 항목에 식별이 용이하도록 이름을 지정합니다.
5. `SSL Mode`에서 `Required`를 선택합니다.
6. 다운로드 받은 번들 인증서 (확장명 .pem 파일)를 텍스트 편집기로 열고 전체 내용을 복사해서 `CA Certificate` 항목에 붙여 넣습니다.
7. `Save` 버튼을 눌러 설정을 저장합니다.
...
6. 우측상단의 `Save Changes` 버튼을 눌러 설정을 저장합니다.
