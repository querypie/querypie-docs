"""reverse_sync/sidecar.py schema v3 유닛 테스트."""
...
class TestSidecarSchema:
    ...
    def test_to_dict_roundtrip_preserves_reconstruction(self):
        original = RoundtripSidecar(
            ...
            blocks=[
                ...
                SidecarBlock(
                    ...
                    "hash_b",
                    (3, 3),
                    reconstruction={"kind": "paragraph", "old_plain_text": "B", "anchors": []},
                ),
            ],
            separators=["\n"],
            document_envelope=DocumentEnvelope(prefix="", suffix="\n"),
        ...
        restored = RoundtripSidecar.from_dict(original.to_dict())

        assert restored.schema_version == ROUNDTRIP_SCHEMA_VERSION
        ...
        assert restored.blocks[1].reconstruction == {
            "kind": "paragraph",
            "old_plain_text": "B",
            "anchors": [],
        }
...
class TestWriteAndLoadSidecar:
    ...
    def test_load_rejects_wrong_version(self, tmp_path):
        ...
        with pytest.raises(ValueError, match=f"expected schema_version={ROUNDTRIP_SCHEMA_VERSION}"):
            load_sidecar(path)
...
class TestBuildSidecar:
    def test_simple_case(self):
        ...
        assert sidecar.blocks[1].reconstruction == {
            "kind": "paragraph",
            "old_plain_text": "Body text",
            "anchors": [],
        }
        assert sidecar.separators == ["\n"]

    def test_list_block_includes_reconstruction_metadata(self):
        xhtml = "<ul><li><p>Parent</p><ul><li><p>Child</p></li></ul></li></ul>"
        mdx = "- Parent\n    - Child\n"
        sidecar = build_sidecar(xhtml, mdx, page_id="test")

        assert sidecar.blocks[0].reconstruction == {
            "kind": "list",
            "old_plain_text": "ParentChild",
            "ordered": False,
            "items": [],
        }

    def test_container_block_records_child_xpaths(self):
        xhtml = (
...
class TestSidecarIdentityHelpers:
    def test_build_sidecar_identity_index_groups_by_hash_in_line_order(self):
        blocks = [
            SidecarBlock(0, "p[1]", "<p>A</p>", "same", (8, 8)),
            SidecarBlock(1, "p[2]", "<p>A</p>", "same", (3, 3)),
            SidecarBlock(2, "p[3]", "<p>B</p>", "other", (5, 5)),
        ]

        index = build_sidecar_identity_index(blocks)

        assert [block.xhtml_xpath for block in index["same"]] == ["p[2]", "p[1]"]

    def test_find_sidecar_block_by_identity_prefers_line_range(self):
        blocks = [
            SidecarBlock(0, "p[1]", "<p>Repeated</p>", "same", (10, 10)),
            SidecarBlock(1, "p[2]", "<p>Repeated</p>", "same", (20, 20)),
        ]

        result = find_sidecar_block_by_identity(blocks, "same", (20, 20))

        assert result is blocks[1]

    def test_find_sidecar_block_by_identity_uses_occurrence_when_range_missing(self):
        blocks = [
            SidecarBlock(0, "p[1]", "<p>Repeated</p>", "same", (10, 10)),
            SidecarBlock(1, "p[2]", "<p>Repeated</p>", "same", (20, 20)),
        ]

        result = find_sidecar_block_by_identity(blocks, "same", None, occurrence_index=1)
