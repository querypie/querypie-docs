...
> 최초 작성일: 2026-03-13
> 갱신일: 2026-03-18 (3차 — Phase 5 상세 계획 반영)
> 기준 브랜치: `main`
...
MDX diff
  ...
  → build_patches(..., mdx_to_sidecar=None, roundtrip_sidecar=sidecar)
      ...
      → container block → 'containing' 전략 → _flush_containing_changes()
                                           # ← text_transfer 잔존 (Phase 5 Axis 1 제거 대상)
  → patch_xhtml()
  → roundtrip verify
```
...
- `_flush_containing_changes()` → `transfer_text_changes()` — callout/container 블록의 'containing' 전략
...
- `expected_status` 기준:
  - `pass`: **29개** — 현재 전부 통과 (PR #937에서 1건 pass로 전환)
  - `fail`: **13개** — 현재 전부 실패 (예상됨)
...
### 4.1 'containing' 전략에서 text_transfer 잔존

`_resolve_mapping_for_change()`는 callout 블록과 children이 있는 블록에 'containing' 전략을 부여하고, 이를 `_flush_containing_changes()` → `transfer_text_changes()`로 처리한다.

Phase 4에서 `reconstruct_container_fragment()`가 이미 구현됐으므로, 'containing' 전략을 reconstruction 경로로 연결하는 작업만 남았다. 이것이 Phase 5 Axis 1의 대상이다.

### 4.2 CLI `record_mapping()` 중복 호출

`reverse_sync_cli.py`는 `record_mapping(xhtml)`을 직접 호출(line 345)하고, `build_sidecar()` 내부에서도 동일 함수를 다시 호출한다. `build_patches()`가 `mappings: List[BlockMapping]`과 `xpath_to_mapping`을 인자로 받는 이유가 이 중복 구조에 있다.

`_resolve_mapping_for_change()`의 'containing' 전략이 sidecar v3 기반으로 전환되면, `mapping.children` 조건도 `sidecar_block.reconstruction.child_xpaths` 기반으로 대체할 수 있다. 그 이후 `build_patches()` 시그니처에서 `mappings`, `xpath_to_mapping`을 제거하고 CLI의 직접 호출도 제거한다. 이것이 Phase 5 Axis 2의 대상이다.
...
| `expected_status: pass` 케이스 유지 | ✅ 29개 통과 |
| unsupported 구조는 silent corruption 없이 명시적 fail | ✅ |
| `mapping.yaml` 없이 sidecar v3만으로 runtime planning 가능 | ⬜ Phase 5 달성 목표 |

---

#### Phase 5 Axis 1: 'containing' 전략 → reconstruction 전환

**목표:** `_flush_containing_changes()` → `transfer_text_changes()` 경로를 제거하고, sidecar v3 기반 `reconstruct_container_fragment()`로 대체한다.

**현재 흐름:**

```
_resolve_mapping_for_change()
  └─ strategy='containing' (callout, children 있는 블록)
       └─ _flush_containing_changes()
            └─ transfer_text_changes(old_plain, new_plain, xhtml_text)
```
...
```
_resolve_mapping_for_change()
  └─ strategy='direct' (with sidecar block)
       └─ _build_replace_fragment_patch()
            └─ reconstruct_fragment_with_sidecar(new_fragment, sidecar_block)
                 └─ reconstruct_container_fragment()   # container
                    reconstruct_inline_anchor_fragment()  # paragraph with anchors
```
...
callout 블록과 children이 있는 블록에서, sidecar block이 존재하고 `container_sidecar_requires_reconstruction()`이 True이면 'containing' 대신 'direct'로 전환한다.
...
if change.old_block.type == 'callout':
    ...
    if sidecar_block and container_sidecar_requires_reconstruction(sidecar_block):
        return ('direct', mapping)
    return ('containing', mapping)          # sidecar 없으면 fallback 유지
if mapping.children:
    sidecar_block = xpath_to_sidecar_block.get(mapping.xhtml_xpath)
    if sidecar_block and container_sidecar_requires_reconstruction(sidecar_block):
        return ('direct', mapping)
...
> `xpath_to_sidecar_block` 딕셔너리(`{xpath: SidecarBlock}`)를 `_resolve_mapping_for_change()`에 전달해야 한다. `build_patches()`에서 이미 구축하므로(line 352–355) 인자로 추가하면 된다.

**Fallback 정책:** sidecar block 없거나 `container_sidecar_requires_reconstruction()`이 False이면 현재 'containing' + `_flush_containing_changes()` 경로를 일시적으로 유지한다. fallback 진입 시 WARNING 로그를 추가해 추적 가능하게 한다.

**Axis 1 완료 기준:**
- [ ] `test_reverse_sync_reconstruction_goldens.py` 12개 통과 유지
- [ ] `expected_status: pass` 29개 유지
- [ ] `pytest tests/` 전체 통과

**Axis 1 완료 후 삭제 대상:**
- `_flush_containing_changes()` (fallback이 0케이스가 되면)
...
#### Phase 5 Axis 2: CLI `record_mapping()` 중복 제거 및 `mappings` 의존 해소

**목표:** `reverse_sync_cli.py`의 `record_mapping()` 직접 호출을 제거하고, `build_patches()`가 sidecar v3만으로 동작하도록 시그니처를 단순화한다.
...
1. `_resolve_mapping_for_change()`에서 `mapping.children` → `sidecar_block.reconstruction.child_xpaths` 기반 판단으로 전환 (Axis 1 이후)
2. `build_patches()` 시그니처에서 `mappings: List[BlockMapping]` 및 `xpath_to_mapping` 제거
3. `reverse_sync_cli.py`에서 `record_mapping()` 직접 호출 제거
4. `SidecarEntry` import를 `patch_builder.py`에서 제거 (`_build_mdx_to_sidecar_from_v3()` 출력을 `SidecarBlock` 직접 참조로 교체)

**Axis 2 완료 기준:**
- [ ] `reverse_sync_cli.py`에 `record_mapping()` 직접 호출 없음
- [ ] `build_patches()` 시그니처에서 `mappings`, `xpath_to_mapping` 제거
...
- `text_transfer.py` — Axis 1 완료 후 제거
- `table_patcher.py` — `is_markdown_table()` 이동 후 제거
- `patch_builder.py` 내부 heuristic strategy 분기 (`_flush_containing_changes`) — Axis 1 완료 후 제거
- `xhtml_patcher.py`의 text-only modify 경로 — Axis 1 이후 검토

**정리 전환 조건 (이미 충족):**

1. ✅ changed golden 16개가 새 reconstruction 경로로 green
2. ✅ `expected_status: pass` 29개 회귀 케이스 유지
...
### 2026-03-18 (3차 갱신) 기준 상태

PR #937 머지로 `main`은 Phase 0–4를 모두 흡수했고, testcase `original.mdx` 재생성까지 완료됐다. **남은 핵심 작업은 Phase 5 (기본 경로 전환 및 legacy 축소) 하나다.**

**Phase 5 Axis별 진행 계획:**

1. **Axis 1 — 'containing' → reconstruction 전환**: `_resolve_mapping_for_change()`에서 callout/container 블록이 sidecar v3 기반 `reconstruct_container_fragment()`를 사용하도록 전환. `transfer_text_changes()` 호출 경로 제거. `text_transfer.py` 삭제.

2. **Axis 2 — CLI 단순화**: `record_mapping()` 중복 호출 제거. `build_patches()` 시그니처에서 `mappings`, `xpath_to_mapping` 제거. `SidecarEntry` import 제거.

3. **Axis 3 — 잔여 legacy 삭제**: `table_patcher.py`, 관련 테스트 정리.

**현재 `expected_status: fail` 13개의 failure_type 분류:**
...
이 중 ft=13, ft=11의 일부는 Phase 5에서 legacy text-transfer를 제거하고 reconstruction 경로로 전환하면 자연히 개선되거나 명확한 fail-closed 처리로 정리될 것으로 예상된다.
